...
const lv_font_t *gui_theme_font(gui_font_role_t role) {
  switch (role) {
  case GUI_FONT_ROLE_TITLE:
    return &app_font_28;
  case GUI_FONT_ROLE_ROW:
    return &app_font_22;
  case GUI_FONT_ROLE_BODY:
    return &app_font_20;
  case GUI_FONT_ROLE_SUBTEXT:
    return &app_font_16;
  case GUI_FONT_ROLE_STATUS:
    return &app_font_16;
  default:
    return &app_font_20;
  }
...
void apply_accent_color(uint32_t rgb) { gui_theme_apply_accent(rgb); }
...
void accent_swatch_event_cb(lv_event_t *e) {
  ...
  for (size_t i = 0; i < ACCENT_PALETTE_COUNT; i++) {
    lv_obj_set_style_border_width(accent_swatches[i],
                                  accent_palette[i] == rgb ? 4 : 0, 0);
  }
}
...
void gui_theme_init(void) {
  init_style_objects();
  fallback_font_init_early(current_settings.font_size_tier,
                           current_settings.lyrics_font_size_tier);
}
...
void gui_theme_reload_styles(void) { init_style_objects(); }
